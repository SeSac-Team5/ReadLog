...
5. ~~**`expo-notifications` 의존성 누락**: `frontend/src/hooks/auth/dailyReminder.ts`(알림 설정 화면이 씀)가 필요로 하는데 없어서 `npx expo install expo-notifications`로 SDK54 호환 버전 추가 — 새 네이티브 모듈이라 `expo prebuild --clean` 다시 돌리고 Xcode 재빌드 필요.~~ → **기능 자체 제거 (2026-07-06)**: 마이페이지 "알림 설정" 메뉴, `NotificationSettingsScreen`, `dailyReminder.ts`, `expo-notifications` 의존성을 전부 제거함 — 안 쓸 기능으로 확정. 아래 `aps-environment` 관련 이슈도 같이 해소됨(패키지 자체가 없으니 오토링킹 문제도 없음).
...
### origin/Ryu 병합 (C 독서모임 기능 + 공용 인프라)

C의 `origin/Ryu` 브랜치를 병합했다. YSE 때와 마찬가지로 이번엔 히스토리가 실제로 이어져 있어서(`dcd5990`을 공통 조상으로 공유) `git merge origin/Ryu`로 처리했다. `dcd5990` 이후 reading_group 관련 파일 대부분은 이미 이전 develop 통합 때 파일 단위로 복사해와서 우리 브랜치에도 있었기 때문에, 실제로 새로 들어온 변경은 생각보다 적었다.

- **가져온 것 (reading_group 로직 개선)**: `progress_service.py`가 진도 응답에 `nickname`을 제대로 채워 반환하도록 수정(`_build_response` 헬퍼 도입, 기존엔 raw ORM 객체를 그대로 반환해서 `nickname`이 비어있었을 것), `MemberProgressBar`에 `memo` 표시 추가, `GroupHomeScreen`이 화면 포커스 시 `fetchProgress`까지 재조회하도록 수정(마이페이지에 적용한 것과 같은 종류의 "포커스 시 새로고침" 개선).
- **키보드 이슈 수정 — origin/Ryu 버전으로 교체**: 직전에 C가 보낸 `keyboardProvider_add.md` 문서를 보고 `KeyboardStickyView` 기반으로 직접 구현했었는데, `origin/Ryu`의 실제 커밋 메시지를 보니(`1cb8c01`) C가 실기기 테스트 후 "`KeyboardStickyView` → `KeyboardAvoidingView`(react-native-keyboard-controller 제공) 전환"으로 최종 변경한 상태였다. 즉 문서는 중간 단계 계획이고 실제 반영된 건 더 뒤 버전 — **우리가 직접 짠 `KeyboardStickyView` 버전을 버리고 `origin/Ryu`의 `KeyboardAvoidingView` 버전으로 완전히 교체**했다. 곁들여 `Comments` 화면을 `GroupStack`(모임 탭 하위)에서 `RootNavigator` 최상위로 옮기고(`headerShown: true`), Android `app.json`에 `softwareKeyboardLayoutMode: "resize"`를 추가한 것도 같은 키보드 수정의 일부라 함께 반영. 이제 안 쓰는 `hooks/reading-group/useKeyboardHeight.ts`는 삭제.
- **공용 인프라 신규 추가**: Android 네이티브 프로젝트 전체(`frontend/android/`, 지금까지 iOS만 있었음), `frontend/eas.json`(EAS 빌드 설정) — 모두 이번에 처음 생김. `app.json`에 `extra.eas.projectId` 추가.
- **`frontend/src/config.ts` 방식 교체**: 기존엔 `app.json`의 `extra.apiBaseUrl`을 `expo-constants`로 읽는 방식이었는데(지난 세션에서 이 경로의 여러 버그를 고생해서 잡았던 바로 그 메커니즘), C의 버전은 `process.env.EXPO_PUBLIC_API_BASE_URL`(폴백 `http://localhost:8000/api`)을 직접 쓰는 더 단순한 방식이다. `.env`에 이미 `EXPO_PUBLIC_API_BASE_URL`이 설정되어 있어서 그대로 호환됐고, `app.json`의 `extra.apiBaseUrl`은 제거했다. 폴백이 (예전처럼 누군가의 로컬 IP가 아니라) `localhost`라서 이 클래스의 스테일 IP 버그가 재발할 여지도 줄어든다 — 팀 전체가 이 방식으로 통일하는 게 낫다고 판단해 채택.
- **`backend/app/core/config.py`**: 안 쓰던 `JWT_SECRET`/`JWT_ALGORITHM`/`JWT_EXPIRE_MINUTES` 필드 제거(코드베이스 전체에서 참조하는 곳이 없어서 확인 후 제거 — 실제 인증은 세션 기반). 로컬 `backend/.env`에도 같은 키들이 남아있어서 `pydantic-settings`가 "정의 안 된 필드"로 막았던 것도 같이 정리(로컬 파일이라 git 추적 안 됨).
- **`babel.config.js`**: `react-native-reanimated/plugin`을 명시적으로 추가 — `babel-preset-expo`가 자동 감지해서 넣어주는 것으로 추정했었는데, C가 명시적으로 넣은 실제 동작 확인된 설정을 그대로 채택.

### 모임 개설 화면 — 진행 도서를 "내 서재"에서 선택 (C·B 협업)

`CreateGroupScreen`(reading-group, C 소유)의 "진행 도서 선택" 버튼이 원래 `navigation.navigate('BookSearch', { pickerMode: true, onBookSelect: ... })`로 reading-plan의 `BookSearchScreen`(알라딘 검색)을 재사용하려던 흔적이 있었는데, 정작 `BookSearchScreen`은 `pickerMode`/`onBookSelect` 파라미터를 전혀 읽지 않아서(내부적으로 항상 `BookDetail`로만 이동) 실질적으로 동작하지 않는 죽은 코드였다. 이번에 "알라딘 검색이 아니라 내 서재에 이미 등록된 책 중에서 실시간 검색으로 고르고 싶다"는 요청으로 아예 새로 구현했다.

- 신규 화면 `frontend/src/screens/reading-group/SelectLibraryBookScreen.tsx`(C 소유 디렉토리) 추가 — `useLibrary()`(reading-plan)로 내 서재 아이템을 가져와 제목/저자 기준 클라이언트 사이드 실시간 필터링(디바운스나 API 호출 없이 `useMemo`로 즉시 필터링, 이미 다 불러온 로컬 배열이라 서버 왕복이 필요 없음). 서재가 비어있으면 안내 문구, 검색 결과가 없으면 별도 문구 표시.
- `CreateGroupScreen`은 `navigation.navigate('SelectLibraryBook', { onSelect })` 콜백 패턴으로 연결(route params에 함수를 직접 넘기는 기존 패턴 유지 — 네이티브 스택 네비게이션은 파라미터를 직렬화하지 않고 메모리에 그대로 들고 있어서 문제없이 동작). 선택된 책 미리보기 카드에 표지 이미지도 추가(기존엔 빈 placeholder 박스만 있었음).
- `book_id`는 reading-plan이 검색/등록 시 채우는 공용 `books` 테이블의 PK를 그대로 참조 — `reading_groups.book_id` FK가 원래도 같은 테이블을 가리키므로 스키마 변경 없이 그대로 맞물린다.
- `navigation/index.tsx`(공용 파일)에 `SelectLibraryBook` 라우트를 `GroupStack`에 등록 — reading-plan의 `LibraryStack`은 건드리지 않음.

### origin/yseSUM 병합 (A 홈 미리보기 + 마이페이지 그룹 활동 탭)

`origin/yseSUM`은 우리 브랜치의 `314bb51`(아이콘 교체 커밋) 지점에서 분기된 A의 계속 작업 브랜치다. A의 로컬이 그 이후 우리가 한 작업(KeyboardProvider 도입, origin/Ryu 병합, config.ts env var 전환, 진행 도서 선택 기능)을 못 받은 상태에서 커밋한 것이라, 이번에도 YSE 때와 똑같은 패턴의 "회귀"가 자동 병합으로 조용히 들어왔다 — 셋 다 충돌 마커 없이 조용히 A쪽으로 덮어써졌길래 병합 후 하나씩 확인하고 되돌림:

- `App.tsx`의 `KeyboardProvider` 제거 → 복원.
- `ReadingProgressScreen.tsx`의 퍼센트 입력/슬라이더 제거(전에도 겪은 바로 그 회귀) → 복원.
- `SNSShareScreen.tsx`의 `react-native-share` → `expo-sharing` 교체(전에 사용자가 명시적으로 "react-native-share 유지"를 선택했던 사안) → 복원.
- `frontend/package.json`도 위 되돌림에 맞춰 `@react-native-community/slider`/`react-native-keyboard-controller`/`react-native-reanimated`/`react-native-share`를 다시 추가하고 `expo-sharing`은 제거.
- `app.json`/`config.ts`의 `apiBaseUrl`도 A의 로컬 IP + 예전 `expo-constants` 방식으로 되돌아가려던 걸 현재 방식(`EXPO_PUBLIC_API_BASE_URL` env var)으로 유지.
- `CommentsScreen.tsx`도 A의 스테일한 RN 코어 `KeyboardAvoidingView`(오프셋 없음) 버전으로 덮일 뻔한 걸 우리 최종 버전(`react-native-keyboard-controller`의 `KeyboardAvoidingView` + `useHeaderHeight`)으로 유지.

**중요한 판단 — 마이페이지 "한줄평"(→"내 코멘트") 탭 데이터 소스**: A가 `progress_service.list_library_comments`를 다시 건드려서 `reading_progress_logs.memo` 대신 `reviews` 테이블(`OneLineReviewScreen`에서 쓰는 진짜 한줄평)을 보여주도록 바꿔놨다. A 코드 주석엔 "이름은 comment지만 실제로는 reviews를 보여줘야 한다"고 되어 있었는데, 이 탭은 이번 세션에서 사용자가 명시적으로 "독서 진도 입력 시 남겼던 코멘트들을 마이페이지 한줄평 탭에 보여달라"고 요청해서 의도적으로 `reading_progress_logs.memo`를 소스로 선택한 것이었다(문서 상단 "마이페이지 한줄평 탭에 진도 코멘트 노출" 섹션 참고). **그 결정을 유지하고 A의 `reviews` 전환은 되돌렸다.** 다만 A가 탭 라벨을 "한줄평" → "내 코멘트"로 바꾼 건 오히려 지금 데이터 소스(진도 코멘트)에 더 잘 맞는 이름이라 그대로 채택했다. 데이터 소스에 대한 이견이 있으면 팀 논의 필요.

**가져온 것 (실제 새 기능)**:
- 알림 기능 전체 제거(`NotificationSettingsScreen.tsx`, `hooks/auth/dailyReminder.ts` 삭제, `expo-notifications` 의존성 제거, 네비게이션 라우트 정리) — A의 auth 모듈 스코프 결정.
- 마이페이지 "독서그룹 활동" 탭 실제 구현(`GroupActivityCard`) — 지금까지 항상 빈 상태였던 탭에 참여 모임 카드(모임명, 인원, D-day, 내 진도%)를 표시. `useMyGroups`/`useMyLatestGroupProgress`(신규 훅, `hooks/reading-group/useGroups.ts`) 사용.
- 홈 화면에 "최근 읽은 책"/"최근 모임" 미리보기 카드 + 연속독서 문구(그룹 댓글 작성 날짜 기준 스트릭 계산) 추가 — 홈은 3개 모듈 데이터를 다 쓰는 공용 화면(CLAUDE.md §3)이라 원래도 여러 모듈에서 손대는 게 정상.
- `UserLibraryItem.updatedAt` 필드 추가(백엔드 스키마+서비스, 프론트 타입) — 홈 화면이 "최근 읽은 책"을 정렬하는 데 사용.

### origin/Ryu 재병합 (C 계속 작업 — 도서커버 연동, 참가 시 서재 자동추가)

C의 `origin/Ryu`가 우리 `df83d8e`(진행 도서 선택 기능) 지점에서 분기해서 커밋 하나(`246a65a`)를 추가한 상태였다 — 즉 C가 이미 우리 최신 작업을 받아간 뒤 시작한 브랜치라 이번엔 A 때와 달리 회귀가 전혀 없었고, 충돌 없이 깨끗하게 병합됐다.
